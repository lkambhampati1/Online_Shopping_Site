...
<?php
include "db.php";
$sql = "SELECT count(cart_id) as cart_count from cart where userid=$_SESSION[user_id];";
$result = $conn->query($sql);
?>
<?php while($row = $result->fetch_assoc()){
  $coun=$row["cart_count"];
...
{
?>
<p style="font-size:40px; font-weight:bold;margin:15px 0px 25px 150px;color:orange;"> Your cart is empty </p>
<?php
}
?>
